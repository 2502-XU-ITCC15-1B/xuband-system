...
$members = dbQuery(
    'SELECT u.id, u.name, u.role, u.instrument, u.year_level,
  ...
  <div class="xu-modal" style="max-width:400px">
    ...
    <div class="xu-modal-body">
      ...
      <div class="row g-2" style="font-size:.88rem">
        ...
        <div class="col-12" id="modalMemberContactWrap" style="display:none">
...
function openMemberModal(m) {
  var avatarHtml = m.avatar_path
    ? '<img src="' + m.avatar_path + '" alt="' + escHtml(m.name) + '" style="width:80px;height:80px;border-radius:50%;object-fit:cover;margin:0 auto">'
    : '<div class="user-avatar mx-auto" style="width:80px;height:80px;font-size:1.8rem">' + escHtml(m.initials) + '</div>';
  ...
  // Contact number — only show row if present
  var contactWrap = document.getElementById('modalMemberContactWrap');
  if (m.contact_number) {
  ...
  // Notes — only show row if present
...
function roleBadgeJs(role) {
  var map = { 'officer': ['bg-warning text-dark', 'Officer'], 'member': ['bg-secondary', 'Member'], 'moderator': ['bg-danger', 'Moderator'] };
